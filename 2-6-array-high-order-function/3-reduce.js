...
// 사과 바구니에 있는 모든 사과들의 당도를 합산해 주세요.
...
/*
색깔 별로 사과 카운트 해서 객체로 만들기
결과 예시 : { `red`: 2, `green`: 4, `yellow`: 3 }
*/

/*let red = 0;
for (let i = 0; i < appleBasket.length; i++) {
    if (appleBasket[i].color === `red`) {
        red++
    }
}
let green = 0;
for (let i = 0; i < appleBasket.length; i++) {
    if (appleBasket[i].color === `green`) {
        green++
    }
}
let yellow = 0;
for (let i = 0; i < appleBasket.length; i++) {
    if (appleBasket[i].color === `yellow`) {
        yellow++
    }
}

const result1 = {red, green, yellow}
console.log(result1);*/

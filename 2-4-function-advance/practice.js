...
// userList의 모든 요소가 조건을 만족하면 true, 하나라도 만족하지 않으면 false를 반환하는 every()함수를 구현하세요.
...
// 모든 사용자가 서울에 사는가?
const result1 = every(function (user) {
...
// userList 중 하나라도 조건을 만족하면 true, 모두 만족하지 않으면 false를 반환하는 some()함수를 구현하세요.
...
// 이름에 '왕'이 포함된 사람이 있는가?
const result3 = some(function (user) {
    return user.userName.includes('왕');
}); // true
console.log(result3);
// 부산에 사는 사용자가 있는가?
